...
//                                    gGpsDataPtr->itow;
    ...
    //
    while(1) {
        OS_Delay( 5*OS_TICKS_PER_SECOND );
...
* @detail Variables computed are (a/r)^(n+2), cos_m(lamda) and sin_m(lambda)
...
* @param [in] nMax Maxumum degree of spherical harmonic secular model
...
*		GVdot - Yearly rate of chsnge in grid variation
* @brief CALLS : none
******************************************************************************/
...
*              at x up to nMax. The lenght must by greater or equal to
*              (nMax+1)*(nMax+2)/2.
